...
export default function SettingsMenu({ onClose, onOpenRewards, onOpenReferrals }: SettingsMenuProps) {
  ...
  const savedPrefs = (user as any)?.revigProfile?.notifPrefs
  const [notifPrefs, setNotifPrefs] = useState<NotifPrefs>({
    allEnabled: savedPrefs?.allEnabled ?? true,
    messages: savedPrefs?.messages ?? true,
    revigReceived: savedPrefs?.revigReceived ?? true,
  ...
  const togglePref = (key: keyof NotifPrefs) => {
    setNotifPrefs(prev => {
      ...
      if (key === 'allEnabled') {
        ...
        next = { allEnabled: allOn, messages: allOn, revigReceived: allOn, friendRequests: allOn, feedActivity: allOn }
      } else {
  ...
  const handleDeleteAccount = async () => {
    if (!confirm('Are you sure you want to delete your Revig account? This cannot be undone.')) return
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-10 safe-bottom">
            ...
            <div className="revig-card overflow-hidden">
              ...
              <div style={{ opacity: notifPrefs.allEnabled ? 1 : 0.4, transition: 'opacity 0.2s', pointerEvents: notifPrefs.allEnabled ? 'auto' : 'none' }}>
                ...
                <SettingRow
                  ...
                  label="Revig Received"
                  sublabel="When a friend sends you a Revig"
                  toggle value={notifPrefs.revigReceived}
                  onPress={() => togglePref('revigReceived')}
                  border
                />
                <SettingRow
          ...
          <p className="text-center text-xs text-white/20 mt-2">Revig · v1.0.0</p>
        </div>
      </div>
    </>
  )
