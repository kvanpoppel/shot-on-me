...
export default function Dashboard({
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const res = await axios.get(`${API_URL}/revig/notifications`, {
  ...
  const firstName = user?.revigProfile?.firstName || user?.firstName || ''
  const lastName = user?.revigProfile?.lastName || user?.lastName || ''
  const profilePic = user?.revigProfile?.profilePicture || user?.profilePicture
