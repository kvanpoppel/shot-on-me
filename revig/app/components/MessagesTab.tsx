...
export default function MessagesTab({ onClose }: { onClose?: () => void }) {
  ...
  const fetchConversations = useCallback(async () => {
    if (!token) return
    try {
      const res = await axios.get(`${API_URL}/revig/messages/conversations`, {
  ...
  const fetchMessages = useCallback(async (convId: string) => {
    ...
    setMsgLoading(true)
    try {
      const res = await axios.get(`${API_URL}/revig/messages/${convId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const fetchFriends = useCallback(async () => {
    if (!token) return
    try {
      const res = await axios.get(`${API_URL}/revig/friends`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  // Real-time: revig-message via window event (from SocketContext)
  useEffect(() => {
    const handleRevigMessage = (e: Event) => {
    ...
    window.addEventListener('revig-message', handleRevigMessage)
    return () => window.removeEventListener('revig-message', handleRevigMessage)
  }, [fetchConversations])
  ...
  useEffect(() => {
    ...
    window.addEventListener('revig-typing', handleTyping)
    return () => { window.removeEventListener('revig-typing', handleTyping); clearTimeout(clearTimer) }
  }, [])

  const emitTyping = useCallback((typing: boolean) => {
    if (!socket || !selectedConvId || !selectedOtherUser) return
    const recipientId = selectedOtherUser?.id || selectedOtherUser?._id
    axios.post(`${API_URL}/revig/messages/${selectedConvId}/typing`,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/revig/messages`, payload,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      if (res.data.message) {
        setMessages(prev => {
  ...
  const startNewConversation = (friend: Friend) => {
    ...
    const convId = `revig_${ids[0]}_${ids[1]}`
      ...
      <div className="max-w-2xl mx-auto flex flex-col" style={{ minHeight: '100%' }}>
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.06)', background: '#1A1A2E' }}>
          ...
          <div className="flex-1">
            ...
            {otherTyping ? 'typing...' : 'Revig friend'}
          </p>
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-t safe-bottom flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.06)', background: '#1A1A2E' }}>
          ...
          <div className="flex items-end gap-2">
            ...
            <textarea
              ...
              placeholder="Message..."
              rows={1}
              className="flex-1 px-4 py-3 rounded-2xl text-sm resize-none border border-white/10 focus:border-lime-revig max-h-24"
