...
export default function FeedTab({ onSendRevig, onViewProfile }: FeedTabProps) {
  ...
  const fetchFeed = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await axios.get(`${API_URL}/revig/feed`, {
  ...
  const handleReaction = async (postId: string, emoji: ReactionEmoji) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/revig/feed/${postId}/reaction`, { emoji }, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const handleDeletePost = async (postId: string) => {
    try {
      await axios.delete(`${API_URL}/revig/feed/${postId}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleEditPost = async (postId: string) => {
    ...
    try {
      await axios.put(`${API_URL}/revig/feed/${postId}`, { content: editContent }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleShare = async (postId: string) => {
    setMenuPostId(null)
    try {
      await axios.post(`${API_URL}/revig/feed/${postId}/share`, {}, { headers: { Authorization: `Bearer ${token}` } })
      // Native share fallback
      if (navigator.share) {
        await navigator.share({ title: 'Check out this Revig!', url: window.location.href }).catch(() => {})
      }
    } catch {
  ...
  const handleReportPost = async (postId: string) => {
    ...
    try {
      await axios.post(`${API_URL}/revig/feed/${postId}/report`, { reason: reportReason }, { headers: { Authorization: `Bearer ${token}` } })
    } catch {
        ...
        {isEditing ? (
          <div className="px-4 pb-3">
            <textarea
              ...
              maxLength={1000}
              autoFocus
              className="w-full px-3 py-2 rounded-xl text-sm border border-white/10 focus:border-lime-revig resize-none"
              style={{ background: '#252540' }}
            />
            <div className="flex gap-2 mt-2">
              <button onClick={() => handleEditPost(postId)} className="revig-btn-primary px-4 py-2 text-xs gap-1"><Check className="w-3 h-3" /> Save</button>
              <button onClick={() => setEditingPostId(null)} className="revig-btn-ghost px-4 py-2 text-xs gap-1"><X className="w-3 h-3" /> Cancel</button>
            </div>
          </div>
        ) : post.content && (
  ...
  return (
    <div className="overflow-y-auto px-4 pt-4 pb-6 max-w-2xl mx-auto w-full" style={{ background: '#1A1A2E', minHeight: '100%' }}>
      ...
      {loading ? (
        ...
        <div className="revig-card py-16 text-center">
          <p className="text-4xl mb-3">✨</p>
          <p className="text-white/40 font-medium">Your feed is empty</p>
          <p className="text-white/25 text-sm mt-1">Send a Revig or share a moment to get started!</p>
          <div className="flex gap-3 justify-center mt-5">
            <button onClick={onSendRevig} className="revig-btn-primary px-5 py-2.5 text-sm">
              Send a Revig
            </button>
            <button onClick={() => setShowComposer(true)} className="revig-btn-ghost px-5 py-2.5 text-sm">
              Share a moment
            </button>
          </div>
        </div>
