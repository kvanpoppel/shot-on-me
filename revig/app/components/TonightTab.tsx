...
export default function TonightTab({ onSendRevig, onClose }: { onSendRevig?: (venueId?: string) => void; onClose?: () => void }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      d.trendingVenues = (d.trendingVenues || []).filter(v => isRevigVenue(v.category))
      setData(d)
    ...
    return (
      <>
        ...
        <div className="fixed inset-0 z-50 flex flex-col overflow-hidden" style={{ background: '#0F0F1E' }}>
          <TonightContent data={data} loading={loading} isEmpty={isEmpty} onSendRevig={onSendRevig} onClose={onClose} />
        </div>
      </>
    )
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-28" style={{ background: '#0F0F1E' }}>
      <TonightContent data={data} loading={loading} isEmpty={isEmpty} onSendRevig={onSendRevig} />
    </div>
  )
}

function TonightContent({ data, loading, isEmpty, onSendRevig, onClose }: {
  data: TonightData | null; loading: boolean; isEmpty: boolean
  onSendRevig?: (venueId?: string) => void; onClose?: () => void
}) {
  return (
        ...
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-black text-white" style={{ fontFamily: 'Poppins, sans-serif' }}>
              What's <span style={{ color: '#C8F135' }}>Reviging</span>
            </h1>
          </div>
          {onClose && (
            ...
            <section>
              ...
              <div className="flex flex-col gap-2">
                {data.friendsOut.map((f, i) => {
                  ...
                  return (
                    <div key={i} className="revig-card p-3 flex items-center gap-3">
                      <Avatar u={f.user} />
                      ...
                      <button
                        onClick={() => onSendRevig?.(f.venue._id)}
            ...
            <section>
              ...
              <div className="flex flex-col gap-2">
                {data.trendingVenues.slice(0, 6).map((v, i) => (
                  <div key={v._id} className="revig-card p-3 flex items-center gap-3">
            ...
            <section>
              ...
              <div className="flex flex-col gap-2">
                {data.eventsTonight.map(ev => (
                  <div key={ev._id} className="revig-card p-4">
