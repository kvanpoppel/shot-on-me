...
import {
  Venue, VenueCategory,
  REVIG_CATEGORIES,
  REVIG_PLACES_TYPES, REVIG_PLACES_KEYWORDS,
  CATEGORY_ICONS, CATEGORY_COLORS, EXCLUDED_CATEGORIES,
} from '../types'

// Revig map style — dark with lime-green accents
const REVIG_MAP_STYLES: google.maps.MapTypeStyle[] = [
...
export default function VenueDiscovery({ onSendRevig, savedGoogleVenues = [], onSavedVenuesChange, initialCategory, onCategoryConsumed }: VenueDiscoveryProps) {
  const { token } = useAuth()
  ...
  const fetchDbVenues = useCallback(async () => {
    setLoading(true)
    try {
      const params: any = { limit: 50, platform: 'revig' }
      if (userLocation) {
      ...
      setDbVenues(all.filter(isRevigVenue))
  ...
  useEffect(() => {
    if (initialCategory && isLoaded) {
      ...
      if (REVIG_CATEGORIES.includes(cat)) {
    ...
    try {
      ...
      if (typeof google !== 'undefined' && google.maps?.places?.Place?.searchNearby) {
        const { places: results } = await google.maps.places.Place.searchNearby({
          ...
          includedPrimaryTypes: [REVIG_PLACES_TYPES[category]],
          maxResultCount: 20,
        } as any)
        ...
        svc.nearbySearch({
          ...
          radius: 8000,
          type: REVIG_PLACES_TYPES[category] as any,
          keyword: REVIG_PLACES_KEYWORDS[category],
        }, (results, status) => {
                ...
                <div className="flex flex-col gap-3">
                  {savedGoogleVenues.map(venue => {
                    ...
                    return (
                      <div
                        key={venue.placeId}
                        className="revig-card overflow-hidden cursor-pointer"
            ...
            {selectedCategory && (
              <div className="px-4 mb-4 flex flex-col gap-4 pb-6">
                {placesLoading ? (
                  ...
                  places.map(place => (
                    <div key={place.placeId} className="revig-card overflow-hidden">
                ...
                <div className="px-4 flex flex-col gap-4 pb-6">
                  {loading ? (
                    ...
                    (displayList as Venue[]).map(venue => {
                      ...
                      return (
                        <div key={venue._id} className="revig-card overflow-hidden" onClick={() => onSendRevig?.(venue._id)}>
                          <div className="w-full h-40 relative overflow-hidden" style={{ background: '#2E2E50' }}>
                            ...
                            <div className="absolute top-3 right-3 flex items-center gap-1.5">
                              {hasPromos && <div className="px-2 py-0.5 rounded-full text-xs font-bold revig-badge">Reviging Now</div>}
                          ...
                          <div className="p-4">
                            ...
                            <div className="flex gap-2 mt-3">
                              <button onClick={e => { e.stopPropagation(); onSendRevig?.(venue._id) }} className="flex-1 py-2.5 rounded-xl text-sm font-bold" style={{ background: '#C8F135', color: '#1A1A2E' }}>
                                Send a Revig
                              </button>
                              <button
                                onClick={async e => {
                                  ...
                                  try {
                                    await axios.post(`${API_URL}/revig/checkins`, { venueId: venue._id, venueName: venue.name }, { headers: { Authorization: `Bearer ${token}` } })
