...
export default function FindFriends({ onClose, onViewProfile }: FindFriendsProps) {
  ...
  const fetchSuggestions = useCallback(async () => {
    if (!token) return
    try {
      const res = await axios.get(`${API_URL}/revig/friends/suggestions`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const fetchPendingRequests = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/revig/friends/requests/pending`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleAccept = async (requestId: string) => {
    try {
      await axios.post(`${API_URL}/revig/friends/requests/${requestId}/accept`, {}, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleDecline = async (requestId: string) => {
    try {
      await axios.post(`${API_URL}/revig/friends/requests/${requestId}/decline`, {}, { headers: { Authorization: `Bearer ${token}` } })
      setPendingRequests(prev => prev.filter(r => r._id !== requestId))
    } catch { /* ignore */ }
  }
  ...
  const handleAdd = async (user: SearchUser) => {
    ...
    try {
      await axios.post(`${API_URL}/revig/friends/${uid}`, {}, { headers: { Authorization: `Bearer ${token}` } })
