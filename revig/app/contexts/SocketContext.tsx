...
export function SocketProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && token && currentUserId) {
      ...
      // Revig-specific real-time events
      newSocket.on('revig-message', (data: any) => {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('revig-message', { detail: data }))
        }
      })

      newSocket.on('revig-typing', (data: any) => {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('revig-typing', { detail: data }))
        }
      })

      newSocket.on('revig-notification', (data: any) => {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('new-notification', { detail: data }))
        }
      })
