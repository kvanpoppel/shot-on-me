...
import SendRevig from './components/SendRevig'
...
function RevigApp() {
  ...
  const handleSendRevig = (venueId?: string) => {
    setPrefillVenueId(venueId)
    setPrefillRecipient(undefined)
    setActiveTab('send')
  }

  const handleSendRevigToFriend = (friend: PrefillRecipient) => {
  ...
  return (
    <div className="flex flex-col overflow-hidden" style={{ background: '#1A1A2E', height: '100dvh' }}>
      ...
      <SearchModal
        ...
        onSendRevig={(venueId) => { setShowSearch(false); handleSendRevig(venueId) }}
      />
      {viewProfileUserId && (
        <FriendProfile
          userId={viewProfileUserId}
          onClose={() => setViewProfileUserId(undefined)}
          onSendRevig={(venueId) => { setViewProfileUserId(undefined); handleSendRevig(venueId) }}
          onSendRevigToFriend={(friend) => { setViewProfileUserId(undefined); handleSendRevigToFriend(friend) }}
        />
      )}
