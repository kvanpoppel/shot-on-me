...
export default function Dashboard() {
  ...
  const atFreeLimit = isFreeTier && activeDeals.length >= 1
      ...
      <div className="pb-24 lg:pb-10 space-y-5">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {atFreeLimit ? (
              <button onClick={() => router.push('/dashboard/settings')}
                className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-primary-500 to-amber-400 px-4 py-2.5 text-sm font-bold text-black hover:opacity-90 transition-all shadow-lg shadow-primary-500/20 min-h-[44px]">
        ...
        {/* ── First-time onboarding checklist (only when no deals + no followers) ── */}
        {!loadingPromos && promotions.length === 0 && followerCount === 0 && (
          <div className="rounded-2xl border border-primary-500/20 bg-primary-500/5 p-5">
            <p className="text-sm font-bold text-primary-500 mb-4">👋 Get your venue live in 3 steps</p>
            <div className="space-y-3">
              {[
                { num: 1, label: 'Complete your venue profile', sub: 'Name, hours, category, photo', href: '/dashboard/settings', done: !!venueName && venueName !== 'Your Venue' },
                { num: 2, label: 'Launch your first deal', sub: 'Use Quick Launch below — takes 5 seconds', href: '/dashboard/promotions?action=new', done: false },
                { num: 3, label: 'Notify your first followers', sub: 'Share your venue QR code to get followers', href: '/dashboard/settings', done: followerCount > 0 },
              ].map(step => (
                <button key={step.num} onClick={() => router.push(step.href)}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-all min-h-[56px] ${step.done ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-white/10 bg-black/40 hover:border-primary-500/30'}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${step.done ? 'bg-emerald-500 text-white' : 'bg-primary-500/20 text-primary-400'}`}>
                    {step.done ? <CheckCircle2 className="w-4 h-4" /> : step.num}
                  </div>
                  <div>
                    <p className={`text-sm font-semibold ${step.done ? 'text-emerald-400 line-through' : 'text-white'}`}>{step.label}</p>
                    <p className="text-xs text-white/30 mt-0.5">{step.sub}</p>
                  </div>
                  {!step.done && <ChevronRight className="w-4 h-4 text-white/20 ml-auto flex-shrink-0" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-start">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="rounded-2xl border border-primary-500/15 bg-black/50 p-4">
              ...
              {atFreeLimit ? (
                <button onClick={() => router.push('/dashboard/settings')}
                  className="mt-2.5 w-full rounded-xl border border-amber-500/30 bg-amber-500/8 py-3 text-xs font-bold text-amber-400 hover:bg-amber-500/15 transition-all flex items-center justify-center gap-1.5 min-h-[44px]">
                  <Crown className="w-3.5 h-3.5" /> Upgrade for more deals
                </button>
              ) : (
                <button onClick={() => router.push('/dashboard/promotions?action=new')}
            ...
            {/* Free tier: show what they're missing, clearly */}
            {isFreeTier && (
              <FeatureGate
                requires="growth"
                message="Unlimited deals, AI copy, guest analytics & auto-posting"
                showPreview={false}
              >
                {/* children never rendered for free tier */}
                <div />
              </FeatureGate>
            )}
