...
import { Menu, X, User, Wallet, Bell, MapPin, Settings, LogOut, Camera, Users, Trophy, Calendar, Gift, Share2, Sparkles, Building2, MessageSquare, Home, ArrowRight, ChevronDown, Mail, Phone } from 'lucide-react'
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <button
                ...
                className="flex items-center space-x-2.5 p-2 rounded-xl hover:bg-primary-500/10 transition-all group"
                aria-label="Account"
              >
                  ...
                  <div className="p-5 bg-gradient-to-br from-primary-500/10 via-primary-500/5 to-transparent border-b border-primary-500/20">
                    ...
                    <div className="bg-black/40 border border-primary-500/20 rounded-xl p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-primary-400/70 text-xs uppercase tracking-wider font-semibold">Wallet Balance</span>
                      ...
                      <div className="flex items-baseline space-x-2">
                        ...
                        {(user?.wallet?.pendingBalance || 0) > 0 && (
                          <p className="text-xs text-primary-400/60">
                            ${(user?.wallet?.pendingBalance || 0).toFixed(2)} pending
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                {menuCategories.map((category, categoryIndex) => (
                  <div key={categoryIndex}>
                    <h3 className="text-xs uppercase tracking-wider text-primary-400/60 font-semibold mb-3 px-4">
                      {category.title}
                    </h3>
                      ...
                      {category.items.map((item, itemIndex) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center justify-between px-4 py-3 text-left rounded-xl transition-all touch-manipulation group ${
                              item.isDestructive
                          ...
                          >
                            <div className="flex items-center space-x-3 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {item.description && (
                                  <p className="text-xs text-primary-400/50 mt-0.5 truncate">{item.description}</p>
                                )}
                              </div>
                            </div>
                            <ArrowRight className={`w-4 h-4 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ${
                              item.isDestructive ? 'text-red-400/60' : 'text-primary-400/60'
                            }`} />
                          </button>
                        )
                      })}
