...
import { MapPin, Star, Users, Sparkles, Loader, X, ExternalLink, Globe, Navigation } from 'lucide-react'
...
const SAVED_KEY = 'som_saved_google_venues'
const GOOGLE_FAV_KEY = 'som_favorite_google_venues'

export function getSavedGoogleVenues(): SavedGoogleVenue[] {
  try { return JSON.parse(localStorage.getItem(SAVED_KEY) || '[]') } catch { return [] }
}
export function saveGoogleVenue(venue: SavedGoogleVenue) {
  const existing = getSavedGoogleVenues()
  if (existing.find(v => v.placeId === venue.placeId)) return // already saved
  const updated = [venue, ...existing]
  localStorage.setItem(SAVED_KEY, JSON.stringify(updated))
}
function removeGoogleVenue(placeId: string) {
  const updated = getSavedGoogleVenues().filter(v => v.placeId !== placeId)
  localStorage.setItem(SAVED_KEY, JSON.stringify(updated))
}
function getGoogleFavIds(): string[] {
  try { return JSON.parse(localStorage.getItem(GOOGLE_FAV_KEY) || '[]') } catch { return [] }
}
function toggleGoogleFav(placeId: string): string[] {
  const favs = getGoogleFavIds()
  const next = favs.includes(placeId) ? favs.filter(f => f !== placeId) : [...favs, placeId]
  localStorage.setItem(GOOGLE_FAV_KEY, JSON.stringify(next))
  return next
}
...
export default function MyVenuesTab({ initialOpenVenueId, onVenueOpened }: MyVenuesTabProps) {
  ...
  // Google saved venues (localStorage)
  const [savedGoogleVenues, setSavedGoogleVenues] = useState<SavedGoogleVenue[]>([])
  const [googleFavIds, setGoogleFavIds] = useState<string[]>([])

  // Auto-open venue passed from search, then clear so it doesn't re-open
  useEffect(() => {
  ...
  useEffect(() => {
    setSavedGoogleVenues(getSavedGoogleVenues())
    setGoogleFavIds(getGoogleFavIds())
  }, [])

  useEffect(() => {
    if (token) fetchFollowedVenues()
  }, [token])
  ...
  const fetchFollowedVenues = async () => {
    try {
      ...
      if (response.data?.followedVenues) {
        const normalized = response.data.followedVenues.map((venue: any) => {
  ...
  // ── Google venue actions ───────────────────────────────────────────────
  const handleRemoveGoogle = (placeId: string) => {
    removeGoogleVenue(placeId)
    setSavedGoogleVenues(prev => prev.filter(v => v.placeId !== placeId))
    setGoogleFavIds(prev => prev.filter(f => f !== placeId))
  }

  const handleToggleGoogleFav = (placeId: string, e: React.MouseEvent) => {
    e.stopPropagation()
    const next = toggleGoogleFav(placeId)
    setGoogleFavIds(next)
  }

  const openGoogleVenue = (venue: SavedGoogleVenue) => {
    const url = venue.website || venue.googleMapsUrl
    window.open(url, '_blank', 'noopener,noreferrer')
  }

  // Sort Google venues: favorites first
  const sortedGoogleVenues = [...savedGoogleVenues].sort((a, b) => {
    const aFav = googleFavIds.includes(a.placeId) ? 0 : 1
    const bFav = googleFavIds.includes(b.placeId) ? 0 : 1
    return aFav - bFav
  })

  // ── SOM followed venues filter ─────────────────────────────────────────
  const filteredFollowed = activeFilter === 'with-promotions'
    ? followedVenues.filter(v => (v.promotions?.filter((p: any) => p.isActive) || []).length > 0)
    : followedVenues

  // ── Show VenueProfilePage overlay ─────────────────────────────────────
  if (selectedVenueId) {
    return (
      <VenueProfilePage
  ...
  return (
    <div className="h-full flex flex-col bg-black">
      ...
      {/* ── Saved Google Venues section ─────────────────────────────── */}
      {sortedGoogleVenues.length > 0 && (
        <div className="border-b border-primary-500/10">
          <div className="px-4 pt-4 pb-2 flex items-center justify-between">
            ...
            <span className="text-xs text-primary-400/50">{sortedGoogleVenues.length}</span>
          </div>

          <div className="flex gap-3 px-4 pb-4 overflow-x-auto scrollbar-hide">
            {sortedGoogleVenues.map(venue => {
              const isFav = googleFavIds.includes(venue.placeId)
              const hasWebsite = !!venue.website
              return (
                <div
                  key={venue.placeId}
                  onClick={() => openGoogleVenue(venue)}
                  className="flex-shrink-0 w-44 bg-black/60 border border-primary-500/20 rounded-2xl overflow-hidden cursor-pointer hover:border-primary-500/50 hover:bg-primary-500/5 transition-all group relative"
                >
                  {/* Cover */}
                  <div className="h-20 bg-primary-500/10 flex items-center justify-center overflow-hidden relative">
                    ...
                    <div className="absolute top-2 right-2 flex flex-col gap-1">
                      <button
                        onClick={(e) => handleToggleGoogleFav(venue.placeId, e)}
                      ...
                      >
                        <Star className={`w-3 h-3 ${isFav ? 'fill-yellow-400 text-yellow-400' : 'text-primary-400/60'}`} />
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleRemoveGoogle(venue.placeId) }}
                        className="w-6 h-6 bg-black/70 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-red-500/30 transition-all"
                      >
                        <X className="w-3 h-3 text-primary-400/60 hover:text-red-400" />
                      </button>
                    </div>
                  </div>
                      ...
                      {activePromos.slice(0, 2).map((promo: any, idx: number) => (
                        <div key={idx} className="bg-primary-500/10 border border-primary-500/20 rounded p-2">
                          <div className="flex items-center justify-between">
                            ...
                            {promo.discount && (
                              <span className="text-green-500 font-bold text-sm">{promo.discount}% OFF</span>
                            )}
                          </div>
                          {promo.description && (
                            <p className="text-primary-400 text-xs mt-1 line-clamp-1">{promo.description}</p>
                          )}
                        </div>
                      ))}
