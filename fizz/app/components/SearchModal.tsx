...
export default function SearchModal({ isOpen, onClose, onViewProfile, onSendFizz }: SearchModalProps) {
  const { token } = useAuth()
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (venueRes.status === 'fulfilled') {
          ...
          setVenues(all.filter((v: any) => isFizzVenue(v.category)).slice(0, 5))
        }
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-8">
          ...
          {!loading && !query && (
            <div className="py-10 text-center">
              ...
              <p className="text-white/30 text-sm">Search for people or Fizz spots</p>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {venues.map((v: any) => (
                  <button
                    key={v._id}
                    onClick={() => { onSendFizz?.(v._id); onClose() }}
                    className="flex items-center gap-3 p-3 rounded-2xl text-left hover:bg-white/5 transition-colors"
                    style={{ background: '#1C1C32' }}
                  >
                    ...
                    <span className="text-xs font-bold flex-shrink-0" style={{ color: '#C8F135' }}>Send Fizz</span>
                  </button>
                ))}
              </div>
            </div>
