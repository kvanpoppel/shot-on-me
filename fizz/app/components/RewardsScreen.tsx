...
export default function RewardsScreen({ onClose }: RewardsScreenProps) {
  ...
  const handleRedeemCash = async () => {
    ...
    try {
      ...
      showToast(`$${res.data.redemption?.cashAmount?.toFixed(2) || '5.00'} added to your Fizz wallet!`)
      fetchAll()
      updateUser({})
    } catch (err: any) {
      showToast(err.response?.data?.message || 'Could not redeem')
        ...
        {userPoints >= 100 && (
          <div className="mx-4 mt-4 p-4 rounded-2xl flex items-center gap-3" style={{ background: 'rgba(200,241,53,0.08)', border: '1px solid rgba(200,241,53,0.25)' }}>
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/40">Added directly to your Fizz wallet</p>
            </div>
            <button
              onClick={handleRedeemCash}
              disabled={redeeming === 'cash'}
              className="fizz-btn-primary px-4 py-2 text-sm disabled:opacity-50"
            >
          ...
          {loading ? (
            ...
            rewards.length === 0 ? (
              ...
              <div className="flex flex-col gap-3 pt-3">
                {rewards.map(r => (
                  <div key={r._id} className="fizz-card p-4" style={{ opacity: r.available && r.canAfford ? 1 : 0.55 }}>
            ...
            myRewards.length === 0 ? (
              ...
              <div className="flex flex-col gap-3 pt-3">
                {myRewards.map(redemption => (
                  <div key={redemption.id} className="fizz-card p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
