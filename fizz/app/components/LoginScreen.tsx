...
const BIOMETRIC_CRED_KEY = 'fizz_biometric_cred_id'
const BIOMETRIC_TOKEN_KEY = 'fizz_biometric_token'
const BIOMETRIC_EMAIL_KEY = 'fizz_biometric_email'
...
export default function LoginScreen({ initialMode = 'signin', onBack }: LoginScreenProps) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (biometricAvailable && !biometricEnabled) {
        const storedToken = localStorage.getItem('fizz_auth_token') || sessionStorage.getItem('fizz_auth_token')
        if (storedToken) {
  ...
  const handleBiometricLogin = useCallback(async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        localStorage.setItem('fizz_auth_token', storedToken)
  ...
  const handleEnableBiometric = useCallback(async () => {
    ...
    setBiometricLoading(true)
    try {
      const savedEmail = localStorage.getItem('fizz_savedEmail') || email
      const userId = new TextEncoder().encode(savedEmail || 'fizz-user')
      const challenge = new Uint8Array(32)
      crypto.getRandomValues(challenge)
      ...
      const credential = await navigator.credentials.create({
        publicKey: {
          challenge,
          rp: { name: 'Fizz', id: window.location.hostname },
          user: {
            id: userId,
            name: savedEmail,
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: '#1A1A2E' }}>
        <div className="w-full max-w-sm text-center">
          ...
          <button
            ...
            className="fizz-btn-primary w-full py-4 text-base mb-3 disabled:opacity-50"
          >
          ...
          {forgotSuccess ? (
            <div className="text-center">
              ...
              <button
                ...
                className="fizz-btn-primary px-8 py-3 text-sm"
              >
                Back to Sign In
              </button>
            </div>
            ...
            <>
              ...
              <form onSubmit={handleForgotPassword} className="flex flex-col gap-4">
                ...
                <button type="submit" disabled={forgotLoading} className="fizz-btn-primary w-full py-4 text-base mt-2 disabled:opacity-50">
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-6 pb-10">
        ...
        {mode === 'signin' && (
          <form onSubmit={handleSignIn} className="flex flex-col gap-4">
            ...
            <button type="submit" disabled={loading} className="fizz-btn-primary w-full py-4 text-base mt-1 disabled:opacity-50">
