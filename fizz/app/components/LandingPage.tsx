...
const FEATURES = [
  ...
  },
  {
    ...
    body: 'Find the best dirty soda shops, coffee bars, and juice spots near you — with live deals and "Fizzing Now" alerts.',
    accent: '#00D4FF',
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background: '#0F0F1E' }}>
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 py-5 safe-top">
        <div className="flex items-center gap-2.5">
          ...
          <span className="fizz-wordmark text-[26px]">
            <span style={{ color: '#C8F135' }}>Fi</span><span style={{ color: '#FF5F57' }}>zz</span>
          </span>
        </div>
        <button onClick={onSignIn} className="fizz-btn-ghost px-5 py-2.5 text-sm">
          Sign In
        </button>
      </nav>
      ...
      <section className="relative z-10 px-6 pt-6 pb-14 text-center">
        ...
        {/* Headline */}
        <h1 className="fizz-wordmark text-[52px] leading-[1.0] tracking-tight mb-5">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl mb-8 text-sm font-semibold" style={{ background: 'rgba(255,95,87,0.12)', color: '#FF9A57', border: '1px solid rgba(255,95,87,0.20)' }}>
          🧋 The dirty soda trend is here — and Fizz is how you share it
        </div>

        {/* CTAs */}
        <div className="flex flex-col gap-3 max-w-[300px] mx-auto">
          <button onClick={onCreateAccount} className="fizz-btn-primary w-full py-[15px] text-[15px] gap-2" style={{ boxShadow: '0 8px 32px rgba(200,241,53,0.30)' }}>
            Get Started Free
            <ArrowRight className="w-4 h-4" />
          </button>
          <button onClick={onSignIn} className="fizz-btn-outline w-full py-[15px] text-[15px]">
            Sign In
          </button>
        </div>
          ...
          <div className="relative z-10 p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="fizz-wordmark text-3xl text-white leading-tight">
                  Dirty Soda<br />
            ...
            <p className="text-sm leading-relaxed mb-5" style={{ color: 'rgba(255,255,255,0.55)' }}>
              ...
              Popular in Utah. Spreading everywhere. Fizz has the best spots.
            </p>
            ...
            <button onClick={onCreateAccount} className="fizz-btn-primary w-full py-3.5 gap-2 text-[14px]" style={{ background: 'linear-gradient(90deg,#FF5F57,#FF9A57)', color: '#fff' }}>
      ...
      ═══════════════════════════════════════════════════ */}
      <section className="relative z-10 px-5 mb-10">
        <h2 className="fizz-wordmark text-3xl text-white text-center mb-6">
          Why <span style={{ color: '#C8F135' }}>Fizz</span>?
        </h2>
      ...
      <section className="relative z-10 px-5 mb-10">
        <div className="p-5 rounded-3xl" style={{ background: '#1C1C32', border: '1px solid rgba(255,255,255,0.07)' }}>
          <h3 className="fizz-wordmark text-xl text-white mb-2">
      ...
      <section className="relative z-10 px-5 pb-16 text-center safe-bottom">
        <div className="rounded-3xl p-7 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#1C2318,#18231C)' }}>
          ...
          <div className="relative z-10">
            <h2 className="fizz-wordmark text-4xl text-white mb-2">
              Ready to <span style={{ color: '#C8F135' }}>Fizz?</span>
            </h2>
            <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.45)' }}>
              Join thousands celebrating every moment — one Fizz at a time.
            </p>
            <button onClick={onCreateAccount} className="fizz-btn-primary w-full max-w-xs mx-auto py-4 text-[15px] gap-2" style={{ boxShadow: '0 12px 36px rgba(200,241,53,0.30)' }}>
              Create Free Account
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
