...
export default function ProfileTab({
  ...
  const quickLinks = [
    ...
    { icon: <Rss className="w-4 h-4" style={{ color: '#FF9A57' }} />, label: 'Fizz Feed', sub: 'See what friends share', onPress: onOpenFeed },
  ...
  return (
    <div style={{ background: '#0F0F1E', minHeight: '100%' }}>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 pb-8">
              ...
              {[
                ...
                <div key={f.label} className="mb-4">
                  ...
                  <input type="text" value={f.value} onChange={e => f.set(e.target.value)} className="w-full px-4 py-3 rounded-2xl text-sm border border-white/10 focus:border-lime-fizz" style={{ background: '#252540' }} />
                </div>
              ))}
              <div className="mb-6">
                <p className="text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: 'rgba(255,255,255,0.35)' }}>Bio</p>
                <textarea value={editBio} onChange={e => setEditBio(e.target.value.slice(0, 160))} rows={3} placeholder="Tell your Fizz friends about yourself..." className="w-full px-4 py-3 rounded-2xl text-sm border border-white/10 focus:border-lime-fizz resize-none" style={{ background: '#252540' }} />
        ...
        <div className="fizz-card p-5 relative overflow-hidden">
          ...
          {fizzProfileIncomplete && (
            <button onClick={openEdit} className="w-full mt-4 py-2.5 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 relative z-10" style={{ background: 'rgba(200,241,53,0.12)', border: '1px dashed rgba(200,241,53,0.35)', color: '#C8F135' }}>
              <Pencil className="w-3.5 h-3.5" /> Set up your Fizz profile
            </button>
          )}
          ...
          <div className="grid grid-cols-4 gap-2 mt-4 relative z-10">
            <div className="text-center py-2.5 rounded-xl" style={{ background: 'rgba(200,241,53,0.08)' }}>
              <p className="text-base font-black" style={{ color: '#C8F135' }}>${(user?.fizzWallet?.balance ?? user?.wallet?.balance ?? 0).toFixed(0)}</p>
      ...
      <div className="px-4 mb-4">
        <button onClick={() => setShowAddFunds(true)} className="w-full fizz-btn-primary py-3 gap-2 text-sm">
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-2">
                {history.map((shot: any, idx) => {
                  ...
                  return (
                    <div key={shot._id || idx} className="fizz-card p-3.5 flex items-center gap-3">
            ...
            {savedVenues.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {savedVenues.map(venue => {
                  ...
                  return (
                    <div key={venue.placeId} className="fizz-card overflow-hidden">
