...
export default function StoriesRow() {
  ...
  const fetchStories = useCallback(async () => {
    if (!token) return
    try {
      const res = await axios.get(`${API_URL}/fizz/stories`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleAddStory = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploadRes = await axios.post(`${API_URL}/fizz/upload`, form, {
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' },
      })
      await axios.post(`${API_URL}/fizz/stories`, {
  ...
  const markViewed = async (storyId: string) => {
    try {
      await axios.post(`${API_URL}/fizz/stories/${storyId}/view`, {}, { headers: { Authorization: `Bearer ${token}` } })
    } catch { /* ignore */ }
  }
      ...
      <div className="flex gap-3 px-4 py-3 overflow-x-auto scrollbar-none">
        ...
        >
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center relative" style={{ background: 'linear-gradient(135deg,#C8F135,#00D4FF)' }}>
            {user?.profilePicture || (user as any)?.fizzProfile?.profilePicture ? (
              <img src={(user as any)?.fizzProfile?.profilePicture || user?.profilePicture} alt="" className="w-full h-full rounded-2xl object-cover opacity-60" />
            ) : null}
