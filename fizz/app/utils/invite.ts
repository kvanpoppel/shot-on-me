/**
 * Invite / referral system for Fizz
 */
...
export function getInviteMessage(userName?: string): string {
  ...
  return `${name}invited you to join Fizz! Gift dirty sodas, coffees & treats to anyone. Discover the best non-alcohol spots near you. Join free now!`
}
...
export async function shareInvite(inviteLink: string, message?: string): Promise<InviteResult> {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: 'Join me on Fizz!', text: shareMessage, url: inviteLink })
